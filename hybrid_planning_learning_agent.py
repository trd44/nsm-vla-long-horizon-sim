...
class HybridPlanningLearningAgent:
    def __init__(self, args:dict, config_file='config.yaml'):
        ...
        self.config['learning'][args.rl_algorithm]['seed'] = args.seed
        self.planner = SymbolicPlanner(self.config['planning'][self.domain])
    ...
    def plan_learn_execute(self):
        ...
        while iteration < self.config['plan_learn_execute']['max_iter'] and not goal_achieved:
            ...
            if plan is None:
                # lauch RL to learn how to reach a plannable state from the initial state
                # TODO: implement this
    ...
    def learn_operator(self, grounded_operator:fs.Action, executed_operators:List[fs.Action]=[]):
        ...
        learner = learning.learner.LLMLearner(env=env_copy, domain=self.domain, rl_algo=args.rl_algorithm, grounded_operator_to_learn=grounded_operator, executed_operators=executed_operators, config=self.config)
        learner.learn_operator()

class HybridLLMPlanningLearningAgent(HybridPlanningLearningAgent):
    def __init__(self, args:dict, config_file='config.yaml'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--baseline', type=bool, default=False, help='whether to use the baseline planner and learning algorithm or the hybrid planner and llm-guided learning algorithm')
    ...
    if args.baseline:
        ...
        agent = HybridLLMPlanningLearningAgent(args)
